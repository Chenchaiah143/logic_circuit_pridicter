...
function drawSchematic(key, ins, outs) {
    ...
    if (key === "not_gate") {
        ...
        // Input Wires
        drawWire("M 45,65 L 140,65", A); // Main A
        drawWire("M 45,155 L 140,155", B); // Main B
        
        // Wires splitting to XOR and AND
        drawWire("M 90,65 L 90,95 L 140,95", A); // A branches
        drawWire("M 110,155 L 110,125 L 140,125", B); // B branches
        
        // Connect outputs
        drawWire("M 205,80 L 350,80", SumDiff); // Sum/Diff output
        drawWire("M 205,140 L 350,140", CarryBorrow); // Carry/Borrow output
        
        // XOR gate (Sum/Diff)
        const xorArc = document.createElementNS(svgNS, "path");
        xorArc.setAttribute("d", "M 133,55 Q 148,80 133,105");
        xorArc.setAttribute("class", "wire wire-neutral");
        svg.appendChild(xorArc);
        drawGateShape("M 140,55 Q 155,80 140,105 Q 170,105 195,80 Q 170,55 140,55 Z", SumDiff === 1);
        drawText(153, 84, key === "half_adder" ? "XOR" : "XOR", "gate-label");
        
        // AND gate (Carry/Borrow)
        // Note: Subtractor has a NOT bubble on the upper input of the AND gate (Borrow = A' . B)
        if (key === "half_subtractor") {
            // NOT circle bubble on gate entry
            const bubble = document.createElementNS(svgNS, "circle");
            bubble.setAttribute("cx", "136");
            bubble.setAttribute("cy", "125");
            bubble.setAttribute("r", "4");
            bubble.setAttribute("class", `gate-shape ${CarryBorrow === 1 ? 'active' : ''}`);
            svg.appendChild(bubble);
        }
        drawGateShape("M 140,115 L 165,115 A 25,25 0 0,1 165,165 L 140,165 Z", CarryBorrow === 1);
        drawText(153, 145, "AND", "gate-label");
        
        // Pin values & texts
        drawText(30, 69, "A");
        drawText(30, 159, "B");
        drawText(360, 84, key === "half_adder" ? "Sum (S)" : "Diff (D)");
        drawText(360, 144, key === "half_adder" ? "Carry (C)" : "Borrow (B)");
        
        drawPinVal(65, 65, A);
        drawPinVal(65, 155, B);
        drawPinVal(280, 80, SumDiff);
        drawPinVal(280, 140, CarryBorrow);
